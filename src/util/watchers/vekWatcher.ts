...
export class VekWatcher {

	private evasionOrchestrator: EvasionOrchestrator;
	private static instance: VekWatcher;

	static start(host: string, port: number, sub_sistema_id: string, periferico_id: string) {
		try {
			VekWatcher.instance = new VekWatcher(host, port, sub_sistema_id, periferico_id);
		} catch (error) {
			console.log('An error occurred while the vek watcher was started ' + error + ` ${VekWatcher.name} -> ${this.start.name}`);
		}
	}
					...
					Object.entries(obj_json).forEach(([key, sensor]) => {
						...
						Object.entries(sensor).forEach(([key2, value]) => {
							if (value.transit_end !== undefined) {
								...
								if (this.evasionOrchestrator === undefined) {
									this.evasionOrchestrator = EvasionOrchestrator.getInstance();
									console.log(`\n\nDeserialize undefined: ${obj_transit_end}.`, '\n');
								}
								this.evasionOrchestrator.vek(obj_transit_end);
